...
// Secret key for JWT
const JWT_SECRET = process.env.JWT_SECRET;//Ensure that the JWT_SECRET is stored securely in production (e.g., in environment variables).
...
// });
app.post('/save-transaction', authenticateToken, (req, res) => {
    const { from_account_id, category_id, to_account_id, type_of_transaction, notes, amount, transaction_date, transaction_time } = req.body;
    const userId = req.user.id; // assuming req.user is set by authenticateToken middleware
    
    if (type_of_transaction === 'TRANSFER') {
        // Insert two transactions: Debit and Credit
        const queries = [
            {
                query: `
                    INSERT INTO transactions (user_id, account_id, category_id, type_of_transaction, notes, amount, transaction_date, transaction_time)
                    VALUES (?, ?, NULL, 'Transfer (Debited)', ?, ?, ?, ?);
                `,
                params: [userId, from_account_id, notes, amount, transaction_date, transaction_time]
            },
            {
                query: `
                    INSERT INTO transactions (user_id, account_id, category_id, type_of_transaction, notes, amount, transaction_date, transaction_time)
                    VALUES (?, ?, NULL, 'Transfer (Credited)', ?, ?, ?, ?);
                `,
                params: [userId, to_account_id, notes, amount, transaction_date, transaction_time]
            }
        ];
        ...
        con.beginTransaction((err) => {
            ...
            for (const { query, params } of queries) {
                con.query(query, params, (err, results) => {
                    if (err) {
                        return con.rollback(() => res.status(500).json({ message: "Error saving transfer", error: err }));
                    }
                    completed += 1;
                    if (completed === queries.length) {
                        con.commit((err) => {
                            if (err) {
                                return con.rollback(() => res.status(500).json({ message: "Transaction commit failed", error: err }));
                            }
        ...
        con.query(query, [userId, from_account_id, category_id, type_of_transaction, notes, amount, transaction_date, transaction_time], (err, results) => {
            if (err) {
                res.status(500).json({ message: "Error saving transaction", error: err });
            } else {
                res.status(201).json({ message: "Transaction saved successfully" });
...
app.get('/get-transactions', authenticateToken, (req, res) => {
    ...
    con.query(query+` ORDER BY transactions.transaction_date DESC, transactions.transaction_time DESC`, queryParams, (err, results) => {
        if (err) {
...
app.get('/get-categories', authenticateToken, (req, res) => {
    const query = `
        SELECT id, category_name, type_of_category
        FROM categories
    `;
    ...
    con.query(query, (err, results) => {
        if (err) {
            ...
            const incomeCategories = results.filter(category => category.type_of_category === 'Income');
            const expenseCategories = results.filter(category => category.type_of_category === 'Expense');
